...
@section('content')
    ...
    <section class="newUserPage">
        <form action="{{ route('gyms.settings.update', $gym->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT');
            <div class="content">
                <div class="row">
    ...
    @push('scripts')
        ...
        <script src="{{ asset('dashboard/js/mapgyms.js') }}"></script>
        ...
        <script>
            function deleteImage(imageId) {
                ...
                var csrf = "{{ csrf_token() }}"
                $.ajax({
                    url: '{{ route('gyms.settings.deleteimage', ['id' => ':id']) }}'.replace(':id', imageId),
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrf
                    },
