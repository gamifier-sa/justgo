...
@section('content')
    <header class="headerOptions">
        ...
        <div class="headerActions">
            {{-- <a class="addNewBtn" href="{{ route('users.gyms.create') }}"><span>+</span> تسجيل جديد</a> --}}
                                ...
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('users.gyms.edit', $user->id) }}"
        ...
        <script>
            $(document).ready(function() {
                $('#endSubscriptionButton').click(function() {
                    ...
                    $.ajax({
                        url: '{{ route('gyms.endSubscription') }}', // Update this URL to match your route
                        type: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': csrf
                        },
        ...
        <script>
            $(document).ready(function() {
                $('#submitForm').click(function() {
                    ...
                    var csrf = "{{ csrf_token() }}"
                    $.ajax({
                        url: '{{ route('gyms.pushToAllusers') }}', // Update this URL to match your route
                        type: 'POST',
                        data: {
                            title: title,
                            messsage: messsage
        ...
        <script>
            $(document).ready(function() {
                $('#submitFormSelectedUsers').click(function() {
                    ...
                    var csrf = "{{ csrf_token() }}"
                    $.ajax({
                        url: '{{ route('gyms.seletedUsers') }}', // Update this URL to match your route
                        type: 'POST',
                        data: {
                            title: title,
